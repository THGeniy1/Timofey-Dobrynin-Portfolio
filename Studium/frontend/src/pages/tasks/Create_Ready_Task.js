...
import { Loading_Spinner, Input_Section, Select_Section, Education_Select, City_Input, 
  ...
  Loading_POP_Element, Error_POP_Element, Help_Button, Right_Button_Desktop_Section,
   Right_Button_Mobile_Section, Row_Input_Section } from '../../elements/main_elements';
  ...
  return (
    <div className="page_container">
      <div className='title_help_row'>
          <h1 className="page_main_title_text">Разместить готовое задание</h1>
...
function Create_Task_Description(props){
  return(
    <div className='page_container'>
      ...
      <Text_Section onChange={props.onChange} placeholder="Опишите работу, процесс ее сдачи: как именно она сдавалась, какие возникали трудности или требования, и на что стоит обратить внимание при сдаче работы" name={'description'} value={props.value.description}/>
    </div>
  );
}
...
function Create_Task_Price_Input(props) {
  ...
  useEffect(() => {
    ...
    const increasedPrice = numericPrice * 1.2;
    const rounded = Math.round(increasedPrice);
  
    setRoundedPrice(rounded);
}, [price]);
  ...
  return (
    <div className='row_space_between_container'>
      ...
      <div className='form_control_wide'>
        ...
        <div className='little_text' id='grey_font'>Покупатель заплатит: {roundedPrice} ₽</div>
      </div>
    </div>
  );
}
...
function Create_Task_Additionally(props){
  return(
    <div className='page_container'>
      ...
      <Row_Input_Section name="Оценка:" placeholder='Полученая оценка за работу' value_name="score" type={'number'} onChange={props.onChange} value={props.value.score}/>
  ...
  return (
    <div className='status_pop_up_container'>
      <span className='main_text'>
        ...
        <br />
        • ИНН (или отметьте, что вы иностранный гражданин)
        <br />
...
function Create_Ready_Task() {
  ...
  const requiredKeys = ["name", "type", "discipline", "description", "education", "tutor", "score", "price"];
  
  useEffect(() => {
    if (!user?.is_inn_locked || !user?.educations || user.educations.length === 0) {
  ...
  useEffect(() => {
    const getTaskData = async () => {
      if (location.state?.data) {
        ...
        try {
          const response = await axios.get(`/api/rt/rcr/${location.state.data}/`, {headers: {'Authorization': `Bearer ${accessToken}`}});       
  ...
  const createNewTask = async (event) => {
    ...
    const isForeign = Boolean(user?.is_foreign_citizen);
    const hasInn = Boolean(user?.has_inn);
    if (!(isForeign ^ hasInn)) {
      handleError('Введите ИНН или отметьте что вы иностранец');
      return;
    }
    ...
    try {
      ...
      await axios.post("/api/rt/cr/", formData, {headers: {'Authorization': `Bearer ${accessToken}`}});  
...
export default Create_Ready_Task;