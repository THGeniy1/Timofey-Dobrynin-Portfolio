...
import {Input_Section, Select_Section, Education_Select, City_Input, Place_Study_Input, 
    Faculty_Input, Direction_Input, LevelSelect} from '../../elements/main_elements';
    ...
    return (
        <div className="side_sticky_container">
            <div className='space_container'>
                <Select_Section fileName="sorting" value_name="sort" value ={props.data?.type} title="Упорядочить" onChange={props.handleChange} />
...
function Sold_Task_Element(props){
    const RemoveIconComponent = remove_image;
    
    const getStatusColor = (status) => {
        switch(status) {
            case 'active':
                return 'active_status_element';
            case 'review':
                return 'review_status_element';
            case 'unpublished':
                return 'unpublished_status_element';
            default:
                return 'unpublished_status_element';
        }
    };
    ...
    return(
        <li className="menu_element">
            {props.isOwner && 
                <>
                    <div className='task_status_element'>
                        <div className='inner_task_status_element' id={getStatusColor(props.data.status)}></div>
                    </div>
                    <button className="remove_button" type="button" onClick={() => props.togglePopUp({popUpId:'deletePop', taskId: props.data.id})}>
...
function User_Sold_Tasks_Search() {
    ...
    return (
        <Search_Function searchURL={`/api/rt/sold/${encodeURIComponent(userId)}`} addData={{ isOwner, userId }}  isExtentedData={true} searchElement={Sold_Task_Element} rightSection={Right_Menu_Task_Section}/>
    );
}
  
  export default User_Sold_Tasks_Search;